...
####################################
# Drop-in replacement for the `erbium-umode` target that implements the
# erbium-visible API on top of an etsoc1 chip. Consumer code keeps
# including `<erbium/isa/*.h>`; the source files under
# `include/erbium-soc1sim/isa/` shadow the native ones when
# this target is linked instead of `erbium-umode`.
#
...
#
# The `erbium/` piece in the consumer-visible `#include <erbium/isa/...>`
# path is injected at build-stage and at install time — the source
# tree has no asymmetric extra nesting vs native erbium / etsoc.
...
####################################
# Create erbium-soc1sim library
####################################
# Stage headers under <build>/erbium-soc1sim-staged-include/erbium/isa/
# so BUILD_INTERFACE consumers see the same `<erbium/isa/...>` path as
# INSTALL_INTERFACE consumers. The `erbium/` piece exists only in the
# staged/installed trees — the source tree keeps the symmetric
# include/erbium-soc1sim/isa/ layout.
...
# INTERFACE library: ships headers, the soc1sim linker script, and
# the ERBIUM_LINKER_SCRIPT property.
...
# whichever backend target got selected.
set_target_properties(erbium-soc1sim PROPERTIES
    ERBIUM_LINKER_SCRIPT "${ERBIUM_SOC1SIM_UMODE_INSTALL_PREFIX}/share/erbium.ld"
)
set_property(TARGET erbium-soc1sim APPEND PROPERTY
    EXPORT_PROPERTIES ERBIUM_LINKER_SCRIPT)

# Install headers under `<prefix>/include/erbium/isa/...` to match
# the consumer's `#include <erbium/isa/...>` path. Strip the leading
...
# Ship the soc1sim linker script. Unlike native erbium, this script
# exposes eight raw SoC region descriptors (region0_base..region7_size)
# as --defsym-overridable PROVIDE symbols and computes the
# __heap_regions table from them.
install(FILES share/erbium-soc1sim/erbium.ld
    DESTINATION ${ERBIUM_SOC1SIM_UMODE_INSTALL_PREFIX}/share)
