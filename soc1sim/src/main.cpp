...
int main(int argc, char** argv) {
  ...
  // Allocate 16MB device buffer for Erbium memory space
  auto* deviceBuf = runtime->mallocDevice(device, kErbiumMemSize);

  // Load ELF into the pre-allocated buffer
  auto elf = read_file(opts.elf_path);
  auto loadResult = runtime->loadCodeTo(stream, deviceBuf, elf.data(), elf.size());
  ...
  std::cout << "Kernel loaded at 0x" << std::hex
            ...
            << std::dec << " (" << kErbiumMemSize / 1024 / 1024 << " MB buffer)\n";
